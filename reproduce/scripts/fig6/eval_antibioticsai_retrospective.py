...
# === Configuration ===
REPO_DIR = Path(INPUT_DIR) / "Model" / "AntibioticsAI"
SUPP_DIR = Path(INPUT_DIR) / "Data" / "retrospective_antibioticsai" / "supplementary"
RESULTS_DIR = Path(OUTPUT_DIR) / "antibioticsai_retrospective"
    ...
    for i in range(n_bins):
        ...
        for m in ['aucroc', 'ap']:
            try:
                ...
                print(f"  ⚠ FALLBACK [antibioticsai_bins]: bin={i} metric={m} failed ({type(_e_m).__name__}); NaN", file=_s_ab.stderr, flush=True)
                row[m] = np.nan
        try:
            row['f1'] = f1_score(yi, (pi > 0.5).astype(int))
        except Exception as _e_f1:
            print(f"  ⚠ FALLBACK [antibioticsai_bins]: bin={i} f1_score failed ({type(_e_f1).__name__}); NaN", file=_s_ab.stderr, flush=True)
            row['f1'] = np.nan
        bins.append(row)
...
def main():
    ...
    for metric in ['aucroc', 'ap', 'f1']:
        try:
            ...
            import sys as _s_ab1
            print(f"  ⚠ FALLBACK [antibioticsai-blactam-pred]: metric={metric} predict_metric failed ({type(e).__name__}: {e}); skipping", file=_s_ab1.stderr, flush=True)
    ...
    for metric in ['aucroc', 'ap', 'f1']:
        try:
            ...
            import sys as _s_ab2
            print(f"  ⚠ FALLBACK [antibioticsai-within-main]: metric={metric} predict_metric failed ({type(e).__name__}: {e}); skipping", file=_s_ab2.stderr, flush=True)
    ...
    try:
        ppv_params, npv_params, p_pos, p_neg = fit_recalibration(cal_data_recal)
        ...
        import sys as _s_ab3
        print(f"  ⚠ FALLBACK [antibioticsai-main-recal]: fit_recalibration failed ({type(e).__name__}: {e}); skipping main_recal CSV", file=_s_ab3.stderr, flush=True)

    # Recalibrate beta-lactam using main test as calibration
    try:
        ppv_params, npv_params, p_pos, p_neg = fit_recalibration(cal_data_main)
        ...
        import sys as _s_ab4
        print(f"  ⚠ FALLBACK [antibioticsai-blactam-recal]: fit_recalibration failed ({type(e).__name__}: {e}); skipping blactam_recal CSV", file=_s_ab4.stderr, flush=True)
