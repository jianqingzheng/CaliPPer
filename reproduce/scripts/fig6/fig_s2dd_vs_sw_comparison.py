...
from _paths import INPUT_DIR, OUTPUT_DIR as _CALIPPER_OUTPUT_DIR  # also adds CaliPPer/ to sys.path
...
# All writes land inside CaliPPer/. Reads of pre-existing predictions/datasets
# come from INPUT_DIR (staged).
OUTPUT_DIR = Path(_CALIPPER_OUTPUT_DIR) / 'deepantigen_retrospective' / 'sw_comparison_figs'
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
RESULTS_DIR = Path(_CALIPPER_OUTPUT_DIR) / 'deepantigen_retrospective'
...
for suffix in ['.pdf', '.png']:
    fig.savefig(str(OUTPUT_DIR / f'supp_fig_s2dd_vs_sw{suffix}'), dpi=DPI, bbox_inches='tight')
