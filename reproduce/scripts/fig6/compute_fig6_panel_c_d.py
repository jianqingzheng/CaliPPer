...
# Panel 1 = published XBCR-net WT test set (full 1293-sample canonical cal)
# Panel 2 = Omicron-era mAbs (21 samples, independent experimental validation)
#
# PERFORMANCE PREDICTION uses the FULL Panel-1 cal including long-antigen (>280 char)
# samples, regenerated by re-running XBCR-net model_rbd_0 inference with long-antigen
# truncation (panel1_cal_full_long_antigens.csv; see regenerate_xbcr_panel1_full_cal.py).
# The default 280-char inference pipeline silently drops these ~240 rows, leaving only
# 1003 and collapsing the S2DD AUROC prediction to ~0.58; the full cal restores the
# manuscript value (AUROC~0.713). NOTE: this does NOT change RECALIBRATION —
# compute_fig6_recal_data.py still uses the 1003-row test_predictions_original.csv,
# which reproduces panels e/k/l bit-exactly (rank-based metrics are insensitive to the
# dropped rows).
print("\n" + "=" * 70)
print("4. XBCR-net — Panel1 WT(cal, full 1293) → Panel2 Omicron(test), ALL methods")
print("=" * 70)

p1_df = pd.read_csv(os.path.join(RESULTS, 'xbcr_retrospective', 'reproduction',
                                   'panel1_cal_full_long_antigens.csv'))
...
# Cal: Panel 1 WT (labels + predictions + distances) — full long-antigen cal
...
cal_d_xb = np.load(os.path.join(RESULTS, 'xbcr_retrospective',
                                  'distance_cache_panel1_full.npz'))['distances'][:len(cal_y_xb)]
