...
def gen_fig3():
    ...
    def _plot_vbias_curves(ax, curves, metric, title, color_key='fold'):
        for curve in curves:
            ...
            if len(bd) >= 4:
                z = np.polyfit(bd, ba, 2)
                xs = np.linspace(bd.min(), bd.max(), 50)
                ax.plot(xs, np.polyval(z, xs), color=color, linewidth=0.6, alpha=0.4)
        # Global fit
        all_d = np.concatenate([np.array(c['bin_d']) for c in curves if c['metric'] == metric])
        all_a = np.concatenate([np.array(c['bin_actual']) for c in curves if c['metric'] == metric])
        if len(all_d) >= 4:
            z = np.polyfit(all_d, all_a, 2)
            xs = np.linspace(all_d.min(), all_d.max(), 100)
            ax.plot(xs, np.polyval(z, xs), 'k-', linewidth=1.5, alpha=0.7)
            se = np.std(all_a - np.polyval(z, all_d))
            ax.fill_between(xs, np.polyval(z, xs)-se, np.polyval(z, xs)+se, color='gray', alpha=0.08)
        ax.set_xlabel('S2DD distance'); ax.set_ylabel(f'Biased {MDISP[metric]}')
