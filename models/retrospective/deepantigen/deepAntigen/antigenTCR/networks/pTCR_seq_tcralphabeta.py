...
class DeepGCN(MessagePassing):
    def __init__(self, args, aggr='add'):
        ...
        self.cdr3a_encoder = Encoder(25, args['hidden_size'], args['depth'], args['k'])
        self.cdr3b_encoder = Encoder(25, args['hidden_size'], args['depth'], args['k'])
        self.peptide_cdr3a_att = MultiHeadAttention(args['hidden_size'], args['heads'])
        self.peptide_cdr3b_att = MultiHeadAttention(args['hidden_size'], args['heads'])
    ...
    def forward(self, peptide_graphs, cdr3a_graphs, cdr3b_graphs):
        peptide_fs = self.peptide_encoder(peptide_graphs)
        cdr3a_fs = self.cdr3a_encoder(cdr3a_graphs)
        cdr3b_fs = self.cdr3b_encoder(cdr3b_graphs)
        peptide_cdr3a_intermap = self.peptide_cdr3a_att(peptide_fs, cdr3a_fs)
        peptide_cdr3b_intermap = self.peptide_cdr3b_att(peptide_fs, cdr3b_fs)
        peptide_cdr3_intermap = torch.cat([peptide_cdr3a_intermap, peptide_cdr3b_intermap],dim=1)
