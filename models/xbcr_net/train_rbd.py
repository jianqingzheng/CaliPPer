...
eps = 1e-7
##############################################################################################
def train(net_core=None, model_path=None,model_num=None,include_light=1,pos_path=None, neg_path=None, batch_size=None,nb_epochs1=None, restore_pre_train = True, selection_criterion='f1'):
    ...
    def compute_f1(conf_mat, eps=eps):
        ...
        for step in range(num_sample//batch_size):
            ...
            if step % nb_print_steps == 0:
                ...
                if acc_max <= avg_acc_val*1.1:
                    ...
                    f1_sel = compute_f1(avg_conf_mat)
                    if selection_criterion == 'f1':
                        should_save = f1_max < f1_sel
                    else:
                        should_save = acc_max < avg_acc_sel
                    if should_save:
                        ...
                        print("latest loss: %f, acc cls: %f, f1: %f" % (avg_loss_sel, avg_acc_sel, f1_sel))
                        print(eval_confusion(avg_conf_mat))
                # else:
                print("minimal loss: %f, acc cls: %f, f1: %f" % (loss_min, acc_max, f1_max))
