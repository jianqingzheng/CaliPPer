...
# access data
def vec_shift(vec,random_shift=False,seq_shift=10):
...
def read_files(folder_dir,suffix='*.xlsx',exclude_str='result'):
    if 'xlsx' in suffix:
        reader = pd.read_excel
        pths = glob.glob(os.path.join(folder_dir, suffix))
        pths = [pth for pth in pths if exclude_str not in pth]
        ...
        csv_pths = glob.glob(os.path.join(folder_dir, "*.csv"))
        xlsx_pths = glob.glob(os.path.join(folder_dir, "*.xlsx"))
        pths = csv_pths + xlsx_pths
        pths = [pth for pth in pths if exclude_str not in pth]
        return [read_table(pth) for pth in pths]

def read_table(folder_pth):
    try:
        return pd.read_csv(folder_pth, encoding='latin-1')
    except:
        return pd.read_excel(folder_pth, engine='openpyxl')