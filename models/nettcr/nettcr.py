...
# Note: Increased peptide max length from 15 to 25 to accommodate all sequences (max=24 in test data)
PEP_MAX_LEN = 25
TCR_MAX_LEN = 30

if chain=='ab':
    pep_train = utils.enc_list_bl_max_len(train_data.peptide, encoding, PEP_MAX_LEN)
    tcra_train = utils.enc_list_bl_max_len(train_data.CDR3a, encoding, TCR_MAX_LEN)
    tcrb_train = utils.enc_list_bl_max_len(train_data.CDR3b, encoding, TCR_MAX_LEN)
    y_train = np.array(train_data.binder)

    pep_test = utils.enc_list_bl_max_len(test_data.peptide, encoding, PEP_MAX_LEN)
    tcra_test = utils.enc_list_bl_max_len(test_data.CDR3a, encoding, TCR_MAX_LEN)
    tcrb_test = utils.enc_list_bl_max_len(test_data.CDR3b, encoding, TCR_MAX_LEN)
