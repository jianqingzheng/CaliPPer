...
# Try to import from torchtext.legacy, fall back to compatibility module
try:
    ...
    # Use compatibility module for newer torchtext versions
    from torchtext_legacy_compat import (
        Pipeline, Dataset, Field, Iterator, Example, RawField, get_tokenizer, Vocab
    )
    # Mock the torchtext.data.utils functions if not available
    is_tokenizer_serializable = lambda x: True
    dtype_to_attr = lambda x: x
...
class Field_modified(RawField):
    ...
    def numericalize(self, arr, device=None):
        ...
        if isinstance(arr, tuple):
            arr, lengths = arr
            # Handle device conversion for lengths tensor
            device_param = None if device == -1 else (f'cuda:{device}' if isinstance(device, int) and device >= 0 else device)
            lengths = torch.tensor(lengths, dtype=self.dtype, device=device_param)
        ...
        # Handle device conversion: torchtext uses -1 for CPU, but torch.tensor needs None or 'cpu'
        if device == -1:
            device_param = None
        elif isinstance(device, int) and device >= 0:
            device_param = f'cuda:{device}'
        else:
            device_param = device
        var = torch.tensor(arr, dtype=self.dtype, device=device_param)
